...
int main( int argc, char *argv[] )
{
  ...
  registration->SetFixedImage(    fixedDicomReader->GetOutput()    );
  registration->SetMovingImage(   movingDicomReader->GetOutput()   );
  fixedDicomReader->Update();

  registration->SetFixedImageRegion(
     fixedDicomReader->GetOutput()->GetBufferedRegion() );
  ...
  initializer->SetFixedImage(  fixedDicomReader->GetOutput() );
  initializer->SetMovingImage( movingDicomReader->GetOutput() );
  ...
  unsigned int maxNumberOfIterations = 1;
  if( argc > 7 )
    {
  ...
  resampler->SetInput( movingDicomReader->GetOutput() );

  FixedImageType::Pointer fixedImage = fixedDicomReader->GetOutput();
